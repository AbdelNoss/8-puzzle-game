...
def task2():
    ...
    selected_scenarios = random.sample(scenarios, 383)  # Randomly select 383 scenarios
    ...
    for i in range(1, 5):  # Heuristics H1 to H4
        ...
        heuristic_data, average_expanded_nodes, average_depths, average_fringe_sizes = run_configurations_task2(selected_scenarios, heuristic_function, heuristic_name)
...
def run_configurations_task2(scenarios, heuristic, heuristic_name):
    ...
    for scenario in scenarios:
        ...
        puzzle = eightpuzzle.EightPuzzleState(scenario)
        problem = eightpuzzle.EightPuzzleSearchProblem(puzzle)
        path, expanded_nodes, depth, fringe_size = search.aStarSearch(problem, heuristic)
...
def task3():
    ...
    scenarios = load_from_csv()
    scenarios = random.sample(scenarios, 383) 
    all_data = []
    averages_data = []
